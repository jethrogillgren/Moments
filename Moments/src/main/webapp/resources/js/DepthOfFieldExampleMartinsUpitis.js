...
window.onload = init;
var container, stats;
var camera, scene, renderer, materials = [], objects = [], singleMaterial, zmaterial = [], parameters, i, j, k, h, color, x, y, z, s, n, nobjects, material_depth, material_base, cubeMaterial;
...
function init() {
	...
	parameters = {
		color : 0xff1100,
		env_map : reflectionCube,
		combine : THREE.MixOperationa,
		reflectivity : 0.3,
	...
	for (i = 0; i < xgrid; i++)
		for (j = 0; j < ygrid; j++)
			for (k = 0; k < zgrid; k++) {
				...
				materials[c].uniforms = Uniforms
						.clone(THREE.ShaderLib['basic'].uniforms);
				if (singleMaterial)
	...
	gui.add(effectController, "aperture", 0.001, 0.2, 0.001).onChange(
			matChanger);
...
function initPostprocessing() {
	...
	postprocessing.rtTextureDepth = new THREE.RenderTarget(window.innerWidth,
			height, pars);
	postprocessing.rtTextureColor = new THREE.RenderTarget(window.innerWidth,
			height, pars);
	...
	postprocessing.quad = new THREE.Mesh(new Plane(window.innerWidth,
			window.innerHeight), postprocessing.materialBokeh);
